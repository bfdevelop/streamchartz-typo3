...
class ClassCacheManager
{
    ...
    public function reBuild()
    {
        ...
        foreach ($GLOBALS['TYPO3_CONF_VARS']['EXT']['streamchartz_api']['classes'] as $key => $extensionsWithThisClass) {
            $extendingClassFound = false;

            $path = ExtensionManagementUtility::extPath('streamchartz_api') . $classPath . $key . '.php';
            if (!is_file($path)) {
            ...
            if ($extendingClassFound) {
                $cacheEntryIdentifier = 'tx_streamchartz_api_' . strtolower(str_replace('/', '_', $key));
                try {
