<?php
namespace Bluforce\StreamchartzApi\Utility;
...
class ClassLoader implements \TYPO3\CMS\Core\SingletonInterface
{
    ...
    public function loadClass($className)
    {
        ...
        require_once(ExtensionManagementUtility::extPath('streamchartz_api') . 'Classes/Utility/ClassCacheManager.php');
    ...
    {
        if (GeneralUtility::isFirstPartOfStr($className, 'Bluforce\\StreamchartzApi\\')) {
            $modifiedClassName = $this->changeClassName($className);
            if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['streamchartz_api']['classes'][$modifiedClassName])) {
                return true;
            }
        }
        return false;
    ...
    {
        return str_replace('\\', '/', str_replace('Bluforce\\StreamchartzApi\\', '', $className));
    }
}